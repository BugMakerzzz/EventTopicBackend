...
def search_eventa(request):
    ...
    nextevent_content = {   # 20201011 提供的意见, 增加预测事件的解释, 二期时候可以做成json文件进行读取
        "无风险事件": "其它",
        "美国在南海挑起争端": "包含美国采取南海“航行自由”行动、美国发表挑起南海争端的言论等挑衅事件",
        "中方采取反制措施": "包含中方在南海进行军事演习、在南海岛礁部署新的军事力量、发布相关的外交声明等相关反制事件",
        "朝鲜采取军事行动等过激行为": "包含朝试射/部署导弹、进行核试验、进行炮击等不利于半岛局势的事件",
        "西方国家针对朝鲜进行制裁": "包含在韩的军事部署、对朝方的外交谴责、对朝方的经济封锁等制裁事件",
        "台湾政局核心人物鼓吹台独": "包含台湾民间、政坛等组织团体掀起台独浪潮等不利于两岸统一的事件",
        "台湾政局发生大规模人事变化": "包含台湾各级政府因选举等行为而发生的较大人事调整事件"
    }
    eventpre_data = {
